...
void benmark(const BitBoard &board, std::string line, Solver &solver) {
    unsigned long long start_time = getTimeMicrosec();
        int best_move = solver.findBestMove(board);
        unsigned long long end_time = getTimeMicrosec();
        std::cout << line << " " << best_move << " " << solver.getNodeCount() << " " << (end_time - start_time);
}
...
int main() {
    ...
    for(int l = 1; std::getline(inputFile, line); l++) {
        BitBoard board;
        board.play(line);
        benmark(board, line, solver);
        std::cout << std::endl;
    }
    return 0;
}